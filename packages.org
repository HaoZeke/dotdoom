...
** Table of Contents :TOC_3_gh:noexport:
  ...
  - [[#org-additions][Org Additions]]
    ...
    - [[#org-gantt][Org Gantt]]
    - [[#no-org-mind-map][NO Org Mind Map]]
    - [[#org-rifle][Org Rifle]]
    - [[#org-cv][Org CV]]
    - [[#essgd][ESSGD]]
    - [[#org-async][Org Async]]
  ...
  - [[#no-matrix-clients][NO Matrix Clients]]
...
** Faster ~eglot~
Using [[https://github.com/jdtsmith/eglot-booster][eglot-booster]], which in turn needs the [[https://github.com/blahgeek/emacs-lsp-booster][binary]].
#+begin_src emacs-lisp :tangle yes
(package!
  eglot-booster
  :recipe (
...
Starfleet answer sidecars are pasted as GitHub-style Markdown, so Org needs a
GFM backend for fenced source blocks and tables.
#+begin_src emacs-lisp :tangle yes
(package! ox-gfm
...
#+END_SRC
*** NO Org Mind Map :noexport:
CLOSED: [2025-07-27 Sun 02:06]
...
#+END_SRC
*** Org-ql (replaces helm-org-rifle)
Structured org queries with vertico integration. Searches headings, content,
tags, and properties across files.
#+BEGIN_SRC emacs-lisp :tangle yes
(package! org-ql)
#+END_SRC
*** Org CV
Not yet on MELPA, but used here to [[https://gitlab.com/zzamboni/vita/-/tree/master?ref_type=heads][great effect]].
#+BEGIN_SRC emacs-lisp :tangle yes
(package! org-cv
  :recipe (:host gitlab
           :repo "Titan-C/org-cv"
...
#+END_SRC
*** ESSGD
Not yet on MELPA, but [[https://github.com/sje30/essgd][very cool]].
#+BEGIN_SRC emacs-lisp :tangle yes
(package! essgd
  :recipe (:host github
           :repo "sje30/essgd"
...
#+END_SRC
*** Org Async
We use async child processes for Org workflows and BibTeX cache refreshes.
#+begin_src emacs-lisp :tangle yes
(package! async)
...
**** Citar
Use Citar for ~org-cite~ insertion and reference actions while keeping ~org-ref~
...
#+END_SRC
*** CapnProto
I ended up making one of my own based off of ~c-mode~. This is temporary, pending [[https://github.com/capnproto/capnproto/pull/2363][gh(capnproto)-2363]].
#+begin_src emacs-lisp :tangle yes
(package! capnp2-mode
  :recipe (:host github
...
#+END_SRC
*** Fish shell
Still haven't found a shell I really like.
#+BEGIN_SRC emacs-lisp :tangle yes
(package! fish-mode)
#+END_SRC
*** Nickel
...
(package! yasnippet-snippets
  :recipe (:host github
           ...
           :files ("*")))
#+END_SRC
** Org and LaTeX enhancements
Tecosaur ecosystem packages for better org-mode and LaTeX export.
#+begin_src emacs-lisp :tangle yes
...
** Chat and protocol-driven buffers

[[https://github.com/karthink/gptel][gptel]] is a chat client; [[https://github.com/xenodium/agent-shell][agent-shell]] drives a tool over the Agent Client
Protocol, so one buffer works for whichever tool is installed rather than
needing a package for each. Backend details are machine-local and live in an
encrypted file under =local/=.
